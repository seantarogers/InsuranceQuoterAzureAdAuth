namespace InsuranceQuoter.Infrastructure.Functions
{
    ...
    public class CosmosClientManager : ICosmosClientManager
    {
        ...
        public async Task<IEnumerable<TDto>> GetItemsAsync<TDto>(string containerId, string databaseId, QueryDefinition queryDefinition)
            where TDto : Dto
        {
        ...
        public async Task<TDto> GetItemAsync<TDto>(string containerId, string databaseId, QueryDefinition queryDefinition)
            where TDto : Dto
        {
            Database database = client.GetDatabase(databaseId);
            Container container = client.GetContainer(database.Id, containerId);
