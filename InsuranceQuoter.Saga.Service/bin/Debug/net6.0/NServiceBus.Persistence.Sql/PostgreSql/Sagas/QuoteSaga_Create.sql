...
create or replace function pg_temp.create_saga_table_QuoteSaga(tablePrefix varchar, schema varchar)
    returns integer as
    $body$
    declare
...
/* WriteCreateIndex CorrelationId */
        ...
        script = 'create unique index if not exists "' || tablePrefix || '_i_3BE74A341C171619F30E4434714308E9BC8E3A4F" on "' || schema || '"."' || tableNameNonQuoted || '" using btree ("Correlation_CorrelationId" asc);';
        execute script;
/* PurgeObsoleteIndex */
...
select pg_temp.create_saga_table_QuoteSaga(@tablePrefix, @schema);
